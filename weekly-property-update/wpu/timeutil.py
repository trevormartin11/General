"""Time / formatting helpers (week windows, date labels, markdown -> html)."""
...
def week_window(reference: datetime | None = None) -> tuple[datetime, datetime]:
    """(start, end) for the current week in local time; start = Mon 00:00."""
    end = to_local(reference or now_local())
    midnight = end.replace(hour=0, minute=0, second=0, microsecond=0)
    start = midnight - timedelta(days=end.weekday())
    return start, end
...
def md_to_html(md_text: str) -> str:
    inner = _markdown.markdown(md_text, extensions=["extra", "sane_lists", "nl2br"])
